...
public:
    ...
    sc_logic( Log_enum r ) { val = test_value(r); }
    ...
    explicit sc_logic(char r) { val = test_value(char_to_logic[r]); }
    explicit sc_logic(long r) { val = char_to_logic[test_value(r)]; }
    explicit sc_logic(int r) { val = char_to_logic[test_value(r)]; }
    explicit sc_logic( bool r ){ val = r?Log_1:Log_0;}
    ...
    sc_logic operator~() const { return sc_logic( not_table[val] ); }

        // Assignment operators from sc_logic and from char
    sc_logic& operator=( Log_enum r ) { val=test_value(r); return *this; }
    sc_logic& operator=( const sc_logic& r ) { val = test_value(r.val); return *this; }
    // this is questionable whether we need assignment from char
    sc_logic& operator=( char r ){ val = test_value(char_to_logic[r]); return *this; }
    sc_logic& operator=( long r ){ val = char_to_logic[test_value(r)]; return *this; }
    sc_logic& operator=( int r ){ val = char_to_logic[test_value(r)]; return *this; }
    sc_logic& operator=( bool r ){ val = r?Log_1:Log_0; return *this; }
    ...
    sc_logic& negate() { val = not_table[val]; return *this; }

        // Comparison operators with sc_logic and with char
    bool operator==( const sc_logic& r ) const { return (val == r.val); }
    bool operator==( char r ) const { return (val == char_to_logic[r]); }
    bool operator!=( const sc_logic& r ) const { return (val != r.val); }
    bool operator!=( char r ) const { return (val != char_to_logic[r]); }

    sc_logic operator !() { return sc_logic(not_table[val]);}

        // Type cast into char
    char to_char() const { return logic_to_char[val]; }

    long to_long() const { return val; }

    bool is_01() const { return val==Log_0 || val==Log_1; }
    void check_01() const;

    bool to_bool() const { check_01(); return val!=Log_0; }
    void print(ostream& os) const { os << to_char();}
