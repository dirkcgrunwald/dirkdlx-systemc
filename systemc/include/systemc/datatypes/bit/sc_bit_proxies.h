...
{
  rep.set_bit(index,
	      sc_logic::and_table[long(s)][const_cast<const X&>(rep)[index]]);
  return *this;
}
...
{
  rep.set_bit(index,
	      sc_logic::or_table[long(s)][const_cast<const X&>(rep)[index]]);
  return *this;
}
...
{
  rep.set_bit(index,
	      sc_logic::xor_table[long(s)][const_cast<const X&>(rep)[index]]);
  return *this;
}
...
sc_range<X>::operator = ( const char* b )
{
  unsigned len = min(length(),(int)strlen(b));
  for(unsigned i=0; i<len; i++)
    if(!reversed())
