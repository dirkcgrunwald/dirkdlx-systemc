...
: m_num( num_ ), m_from( from_ ), m_to( to_ ),
  m_bv( *new sc_bv_base( max( m_from, m_to ) - min( m_from, m_to ) + 1 ) )
{}
...
{
    SC_ERROR_IF_( i < 0 || i >= m_params.wl(), SC_ID_OUT_OF_RANGE_ );
...
{
    SC_ERROR_IF_( i < 0 || i >= m_params.wl(), SC_ID_OUT_OF_RANGE_ );
    SC_ERROR_IF_( j < 0 || j >= m_params.wl(), SC_ID_OUT_OF_RANGE_ );
